...
Level::Level(std::string path, entityx::EntityX& entityx, sf::RenderWindow& window) {
  ...
  if (!map.load(path)) {
	...
	for (int y = 0; y < mapSize.y; y++) {
	  ...
	  */
      ...
      auto testLevel = entityx.entities.create();
...
void Level::handleObjectGroup(const tmx::ObjectGroup& layer, entityx::EntityX& ex, sf::RenderWindow& window) {
